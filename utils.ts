...
const ROOT = 'https://raw.githubusercontent.com/JamesReynolds/platformer-tutorial/main/';
...
{
    const newShade = { x: Math.floor(Math.random() * 4), y: Math.floor(Math.random() * 4) };
    if (shade && newShade.x === shade.x && newShade.y === shade.y) {
        return randomShade(shade);
    }
    return newShade;
}
